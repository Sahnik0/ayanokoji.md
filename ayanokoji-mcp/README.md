...
An MCP server that serves Ayanokoji's lazy-senior-dev instructions. It exposes
the same ruleset the Claude hooks and Pi extension use, so every host emits
identical rules.

It is not a replacement for the always-on adapters. Ayanokoji normally lives in
...
- Prompt `ayanokoji`, returns the ruleset as a user message. Optional `mode`
  argument: `lite`, `full`, or `ultra`. Omit it to use the configured default.
- Tool `ayanokoji_instructions`, same text, plus `structuredContent`
  (`{ mode, instructions }`), for hosts that pull context via tools or code
  execution. Read-only.

Mode resolution reuses `hooks/ayanokoji-config.js`, so `AYANOKOJI_DEFAULT_MODE`
and `~/.config/ayanokoji/config.json` work the same as everywhere else.
...
```json
{ "mcpServers": { "ayanokoji": { "command": "node", "args": ["ayanokoji-mcp/index.js"] } } }
```
