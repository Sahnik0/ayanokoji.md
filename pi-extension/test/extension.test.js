...
import ayanokojiExtension from "../index.js";
...
function withTempConfig(fn) {
  const tempConfigHome = mkdtempSync(join(tmpdir(), "ayanokoji-test-"));
...
test("extension registers Ayanokoji commands", () => {
...
test("/ayanokoji updates session mode and injects instructions", async () => withTempConfig(async () => {
  const { commands, events, appendedEntries } = createPiHarness();
  const ctx = createCommandContext();

  await events.get("session_start")({ reason: "startup" }, ctx);
  await commands.get("ayanokoji").handler("ultra", ctx);

  assert.deepEqual(appendedEntries.at(-1), {
    customType: "ayanokoji-mode",
    data: { mode: "ultra" },
  });

  const result = await events.get("before_agent_start")({ systemPrompt: "BASE" }, ctx);
  assert.ok(result.systemPrompt.includes("AYANOKOJI MODE ACTIVE"));
...
test("skill alias commands delegate to Pi skill commands", async () => {
  ...
  await commands.get("ayanokoji-review").handler("", ctx);
  await commands.get("ayanokoji-audit").handler("", ctx);
  await commands.get("ayanokoji-debt").handler("", ctx);
  await commands.get("ayanokoji-gain").handler("", ctx);
  await commands.get("ayanokoji-help").handler("", ctx);
  await commands.get("ayanokoji-task").handler("some task", ctx);
  await commands.get("ayanokoji-status").handler("", ctx);
  await commands.get("ayanokoji-gate").handler("", ctx);

  assert.deepEqual(sentUserMessages.map((entry) => entry.text), [
    "/skill:ayanokoji-review",
    "/skill:ayanokoji-audit",
    "/skill:ayanokoji-debt",
    "/skill:ayanokoji-gain",
    "/skill:ayanokoji-help",
    "/skill:ayanokoji-task some task",
    "/skill:ayanokoji-status",
    "/skill:ayanokoji-gate",
  ]);
});
...
test("a request mentioning normal mode stays active", async () => withTempConfig(async () => {
  ...
  const result = await events.get("before_agent_start")({ systemPrompt: "BASE" }, ctx);
  assert.match(result.systemPrompt, /AYANOKOJI MODE ACTIVE/);
}));
...
test("status bar renders the mode and flips active on agent_start", async () => withTempConfig(async () => {
  ...
  const statusWrites = [];
  const ctx = createCommandContext({
    sessionManager: { getEntries: () => [{ type: "custom", customType: "ayanokoji-mode", data: { mode: "ultra" } }] },
  ...
  assert.equal(statusWrites.at(-2).key, "ayanokoji");
