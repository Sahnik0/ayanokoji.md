...
const temp = fs.mkdtempSync(path.join(os.tmpdir(), 'ayanokoji-uninstall-'));
...
const flagPath = path.join(claudeDir, '.ayanokoji-active');
fs.writeFileSync(flagPath, 'full');

const configDir = path.join(temp, 'config-home', 'ayanokoji');
...
fs.writeFileSync(settingsPath, JSON.stringify({
  statusLine: { type: 'command', command: 'bash /some/path/ayanokoji-statusline.sh' },
}));
...
assert.equal(
  ...
  undefined,
  'ayanokoji statusLine entry must be removed',
);
