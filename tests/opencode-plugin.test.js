...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'ayanokoji-opencode-'));
process.env.XDG_CONFIG_HOME = tmp;
delete process.env.AYANOKOJI_DEFAULT_MODE;
const statePath = path.join(tmp, 'opencode', '.ayanokoji-active');

let loadPlugin, parseCommandFile;
test.before(async () => {
  const url = pathToFileURL(path.join(__dirname, '..', '.opencode', 'plugins', 'ayanokoji.mjs'));
...
test('system.transform injects the ruleset at the default mode (full)', async () => {
  ...
  assert.match(system[0], /AYANOKOJI MODE ACTIVE — level: full/);
  assert.match(system[0], /calculating strategist/i);
});

test('command.execute.before persists /ayanokoji ultra, transform follows it', async () => {
  const hooks = await loadPlugin({});
  await hooks['command.execute.before']({ command: 'ayanokoji', arguments: 'ultra', sessionID: 's' });
  assert.equal(fs.readFileSync(statePath, 'utf8'), 'ultra');
  const system = await transform(hooks);
  assert.match(system[0], /AYANOKOJI MODE ACTIVE — level: ultra/);
});

test('/ayanokoji off persists off and transform injects nothing', async () => {
  const hooks = await loadPlugin({});
  await hooks['command.execute.before']({ command: 'ayanokoji', arguments: 'off', sessionID: 's' });
